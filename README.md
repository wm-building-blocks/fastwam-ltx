...
├── scripts/
│   ├── train.py
│   ├── train_zero1.sh        # Deepspeed zero1 training entrypoint (legacy 5B backbone)
│   ├── train_zero2.sh        # Deepspeed zero2 training entrypoint (A14B 14B backbone)
...
Step 2: pre-generate the ActionDiT backbone (interpolated from the Wan2.2-T2V-A14B high-noise expert):

```bash
python scripts/preprocess_action_dit_backbone.py \
  --model-config configs/model/fastwam.yaml \
  --output checkpoints/ActionDiT_linear_interp_A14B_alphascale_1024hdim.pt \
  --device cuda \
  --dtype bfloat16
```

> **Backbone swap note (2026-05-13):** This codebase has been swapped from Wan2.2-TI2V-5B to **Wan2.2-T2V-A14B (high-noise expert only)**. Only the `high_noise_model/` shards are downloaded from the A14B HF repo. The companion VAE is **Wan2.1-VAE** (`z_dim=16`, stride 4x8x8), not the high-compression Wan2.2-VAE used by TI2V-5B. Training uses DeepSpeed Zero-2 via `scripts/train_zero2.sh` (see Training section).
...
```bash
# LIBERO  (DeepSpeed Zero-2, A14B 14B backbone)
bash scripts/train_zero2.sh 8 task=libero_uncond_2cam224_1e-4

# RoboTwin (DeepSpeed Zero-2, A14B 14B backbone)
bash scripts/train_zero2.sh 8 task=robotwin_uncond_3cam_384_1e-4
```

The 14B backbone (`Wan2.2-T2V-A14B` high-noise expert) uses DeepSpeed Zero-2 (shard grads + optimizer states) combined with `bnb.optim.AdamW8bit` to fit on 8x 80GB GPUs while keeping params replicated (required because MoT mixed-attention reads `block.modulation` directly and can't run under param-sharding).
The task configs ship with `batch_size=2 grad_accum=8` (RoboTwin) and `batch_size=4 grad_accum=4` (LIBERO).
Activation checkpointing (`mot_checkpoint_mixed_attn`) is opt-in via task yaml; when GPU memory allows, disabling it gives the largest single speedup.

The legacy DeepSpeed Zero-1 launcher (`scripts/train_zero1.sh`) remains for the older 5B backbone but should not be used with the current A14B configs.
