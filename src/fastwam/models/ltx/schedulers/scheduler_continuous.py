...
class WanContinuousFlowMatchScheduler:
    ...
    def __init__(self, num_train_timesteps: int = 1000, shift: float = 5.0, eps: float = 1e-10, sigma_floor: float = 0.0):
        if num_train_timesteps <= 0:
            raise ValueError(f"`num_train_timesteps` must be positive, got {num_train_timesteps}")
        if shift <= 0:
            raise ValueError(f"`shift` must be positive, got {shift}")
        if not 0.0 <= sigma_floor < 1.0:
            raise ValueError(f"`sigma_floor` must be in [0, 1), got {sigma_floor}")
    ...
    def sample_training_t(
        ...
        **kwargs,
        ...
        if self.sigma_floor > 0.0:
            # Rescale u to span only [sigma_floor, 1] region.
            # Solve phi(u, shift) = sigma_floor => u_floor = s / (shift - (shift-1)*s)
            s = self.sigma_floor
            sh = self.shift
            u_floor = s / (sh - (sh - 1.0) * s)
            u = u_floor + u * (1.0 - u_floor)
