...
from .helpers.loader import load_wan22_a14b_high_noise_components
from .mot import MoT
...
class FastWAM(torch.nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        # Register experts only via `mot.mixtures` so FSDP autowrap traverses each
        # DiTBlock exactly once. `video_expert` / `action_expert` / `dit` are exposed
        # as properties pointing at the same objects (see below).
        if mot.mixtures["video"] is not video_expert or mot.mixtures["action"] is not action_expert:
            raise ValueError("`mot.mixtures` must reference the same expert objects passed in.")
        self.mot = mot
    ...
    @property
    def dit(self):
        # Trainer's optimizer / freeze logic uses `model.dit`; alias to the MoT root.
        return self.mot

    @classmethod
    def from_a14b_high_noise_pretrained(
        cls,
        device: str = "cuda",
        ...
        if video_dit_config is None:
            raise ValueError("`video_dit_config` is required for FastWAM.from_a14b_high_noise_pretrained().")
        if "text_dim" not in video_dit_config:
            raise ValueError("`video_dit_config['text_dim']` is required for FastWAM.")

        components = load_wan22_a14b_high_noise_components(
            device=device,
        ...
        mot = MoT(
            ...
            mot_checkpoint_stride=mot_checkpoint_stride,
        )
    ...
    def build_inputs(self, sample, tiled: bool = False):
        ...
        spatial_stride = self.vae.upsampling_factor
        if height % spatial_stride != 0 or width % spatial_stride != 0:
            raise ValueError(
    ...
    def infer_joint(
        ...
        checked_h, checked_w, checked_t = self._check_resize_height_width(height, width, num_video_frames, spatial_stride=self.vae.upsampling_factor)
    ...
    def infer_action(
        ...
        _, _, height, width = input_image.shape
        if height % self.vae.upsampling_factor != 0 or width % self.vae.upsampling_factor != 0:
            raise ValueError(
                f"`input_image` must be resized before infer, expected multiples of {self.vae.upsampling_factor} (VAE stride) but got HxW=({height},{width})"
            )
        if proprio is not None:
            if self.proprio_dim is None:
                raise ValueError("`proprio` was provided but `proprio_dim=None` so `proprio_encoder` is disabled.")
